...
export default function Footer({ onBooking }) {
  return (
    <footer className="relative bg-void border-t border-white/[0.05] overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,rgba(201,150,12,0.04)_0%,transparent_60%)] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-2 mb-4">
              <span className="font-heading text-2xl italic text-gold-light">Elite</span>
            ...
            <button onClick={onBooking}
              className="btn-gold text-[13px] px-6 py-3 rounded-full cursor-pointer">
          ...
          {/* Links */}
          <div>
            <div className="text-[10px] font-semibold tracking-[0.18em] uppercase text-gold mb-5">Navigation</div>
          ...
          {/* Contact */}
          <div>
            <div className="text-[10px] font-semibold tracking-[0.18em] uppercase text-gold mb-5">Contact</div>
            <ul className="space-y-4">
              {CONTACT.map((c, i) => (
                <li key={i} className="flex items-start gap-3">
                  <c.icon size={14} className="text-gold mt-0.5 flex-shrink-0" strokeWidth={1.5} />
            ...
            <div className="flex items-center gap-3 mt-8">
              {[Linkedin, Twitter, Instagram].map((Icon, i) => (
                <a key={i} href="#" aria-label="Social"
                  className="btn-ghost-gold w-9 h-9 rounded-full flex items-center justify-center cursor-pointer">
